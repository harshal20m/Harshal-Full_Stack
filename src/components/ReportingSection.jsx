	...
	return (
		<div className="bg-white p-4 rounded-3xl w-full shadow">
			<div className="flex justify-between px-2 items-center mb-4">
				<h2 className="text-xl font-semibold">Reporting Section</h2>
			...
			<div className="grid grid-cols-2 w-full">
				{reports.map((report, index) => (
					<div key={index} className="flex mt-1 items-center gap-2 w-full">
						<img src={report.img} alt="" className="w-12 h-12  rounded-full" />
						<div className="flex flex-col ">
							<span className="text-xs font-semibold text-gray-800 ">{report.role}</span>
							<span className="text-xs text-gray-600">{report.name}</span>
						</div>
					</div>
				))}
			</div>
